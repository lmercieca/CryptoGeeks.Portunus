...
namespace P2PChat
{
    public class Client
    {
        public IPEndPoint ServerEndpoint = new IPEndPoint(IPAddress.Parse("13.81.63.14"), 50);
        ...
        private IPAddress GetAdapterWithInternetAccess()
        {
            foreach (NetworkInterface item in NetworkInterface.GetAllNetworkInterfaces())
            {
                ...
                string[] IPAddresses = item.GetIPProperties().UnicastAddresses.Select(x => x.Address.ToString()).ToArray(); ;
        ...
        public IPEndPoint ConnectToClient(ClientInfo CI)
        {
            ...
            for (int ip = 0; ip < CI.InternalAddresses.Count; ip++)
            {
                ...
                for (int i = 1; i < 4; i++)
                {
                    ...
                    if (Responce != null)
                    {
                        ...
                        Message M = new Message(CI.Name, CI.Name, "pi po");
                        SendMessageUDP(M, EP);
            ...
            if (CI.ExternalEndpoint != null)
            {
                ...
                for (int i = 1; i < 100; i++)
                {
                    ...
                    if (Responce != null)
                    {
                        ...
                        Message M = new Message(CI.Name, CI.Name, "pi po");
                        SendMessageUDP(M, CI.ExternalEndpoint);
